package cn.wode490390.nukkit.shpop.populator;
...
import cn.wode490390.nukkit.shpop.StrongholdPlugin;
import cn.wode490390.nukkit.shpop.math.BoundingBox;
import cn.wode490390.nukkit.shpop.scheduler.CallbackableChunkGenerationTask;
import cn.wode490390.nukkit.shpop.structure.StrongholdPieces;
import cn.wode490390.nukkit.shpop.structure.StructurePiece;
import cn.wode490390.nukkit.shpop.structure.StructureStart;
        ...
        if (VALID_BIOMES[chunk.getBiomeId(7, 7)]) {
            ...
            synchronized (this.lock) { //\\ _Mtx_lock((char *)this + 456);
                if (!this.isSpotSelected) {
                    ...
                    if (StrongholdPlugin.DEBUG) {
                        StringJoiner joiner = new StringJoiner("\n");
                        for (long hash : this.strongholdPos) {
                            joiner.add((Level.getHashX(hash) << 4) + "," + (Level.getHashZ(hash) << 4));
                        }
                        StrongholdPlugin.debug(this.getClass().getSimpleName(), "StrongholdFeature::generatePositions\n", joiner);
                    }
                }
            ...
            for (long chunkPos : strongholdPos) {
                if (hash == chunkPos) {
                    ...
                    if (start.isValid()) { //TODO: serialize nbt
                        ...
                        StrongholdPlugin.debug(this.getClass().getSimpleName(), chunkX << 4, chunkZ << 4);
                    }
